...
class _LocalFileByteReceiveStream(ByteReceiveStream):
    """
    ...
    async def _ensure_open(self) -> None:
        """Lazily open the file and seek to start position."""
    ...
    async def receive(self, max_bytes: int = 65536) -> bytes:
        ...
        bytes_to_read = min(max_bytes, self._end - self._position)
        chunk = await self._file.read(bytes_to_read)
...
class AsyncFilesystem(AbstractAsyncContextManager["AsyncFilesystem"]):
    ...
    async def read_file_bytes(
        ...
        if is_s3_filename(filename):
            ...
            return _BytesByteReceiveStream(data)
        else:
            return _LocalFileByteReceiveStream(filename, start, end)
